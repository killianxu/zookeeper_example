        ...
        public boolean execute() throws KeeperException, InterruptedException {
            do {
                if (id == null) {
                    ...
                    //查找当前节点,没有则创建, id = zookeeper.create(dir + "/" + prefix, data,getAcl(), EPHEMERAL_SEQUENTIAL);
                ...
                if (names.isEmpty()) {
                    ...
                    ownerId = sortedNames.first().getName(); //编号最小的节点为锁节点
                    SortedSet<ZNodeName> lessThanMe = sortedNames.headSet(idName);
                    if (!lessThanMe.isEmpty()) {
                        ZNodeName lastChildName = lessThanMe.last();
                        lastChildId = lastChildName.getName();//比当前节点小的前一个节点
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("watching less than me node: " + lastChildId);
                        }
                        //watch比当前节点编号小的前一个节点,当前一个节点发生变化,会触发事件,重新判断是否可以获得锁,不监控所有节点,避免惊群
